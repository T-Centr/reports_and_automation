...
img = 'data:image/png;base64,' + binascii.b2a_base64(
    img.getvalue(), newline=False).decode("UTF-8")
...
pdfkit.from_string(html, 'ege.best.pdf', configuration=config, options=options)

with open("ege.best.html", "w", encoding="utf-8") as file:
    file.write(html)

letter = MIMEMultipart()
letter["From"] = "T-Centr@yandex.ru"
...
attachement.add_header(
    "Content-Disposition", "attachement", filename="ege.best.pdf"
)
...
server.sendmail(
    "T-Centr@yandex.ru", "support@ittensive.com", letter.as_string()
)
